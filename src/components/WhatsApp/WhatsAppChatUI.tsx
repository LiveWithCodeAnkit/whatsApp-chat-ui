...
import { Smile, Paperclip, Mic, MoreVertical, Search, Send, X, ChevronLeft, Edit, Check, ChevronRight } from "lucide-react"
      ...
      <div className={`w-full md:w-1/3 lg:w-1/4 bg-white border-r flex flex-col ${showChatArea ? 'hidden md:flex' : 'flex'}`}>
        ...
        <ScrollArea className="flex-grow">
          {filteredContacts.map(contact => (
            <div
              key={contact.id}
              className={`flex items-center p-3 cursor-pointer hover:bg-gray-100 ${selectedContact.id === contact.id ? "bg-gray-100" : ""
                }`}
              onClick={() => {
          ...
          <div className="flex space-x-2">
            <div className="relative hidden md:block">
              <Input
                ...
                onChange={(e:any) => setChatSearchQuery(e.target.value)}
              />
              ...
              {editingMessageId === message.id ? (
                <div className="flex items-center justify-end space-x-2">
                  <Input
                    ...
                    onChange={(e:any) => setEditMessageContent(e.target.value)}
                    className="flex-grow"
                  />
                ...
                <div
                  className={`inline-block p-2 rounded-lg ${message.sender === "You" ? "bg-green-100" : "bg-white"
                    }`}
                >
                  <p>{message.content}</p>
